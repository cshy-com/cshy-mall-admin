  ...
  <el-dialog v-model="dialogFormVisible" :title="id?'修改核销员':'添加核销员'"  :visible.sync="dialogFormVisible" width="750px" @close="cancel">
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm" @submit.native.prevent v-loading="loading">
      ...
      <el-form-item label="所属提货点：" prop="storeId">
        <el-select v-model="ruleForm.storeId" placeholder="请选择"  multiple  style="width:50%" clearable>
          <el-option
...
<script>
  ...
  import {   systemAdminGetId } from '@/api/user';
    ...
    export default {
      ...
      methods:{
        ...
        upImgUid(row){
          ...
          this.getAccInfo( this.ruleForm.uid)
        },
        upImg(){
        ...
        getAccInfo(){
          systemAdminGetId( this.ruleForm.uid).then(res=>{
        ...
        getInfo (id) {
          ...
          storeStaffInfoApi({id:id}).then(res=>{
            ...
            res.storeId.forEach(val=>{
                
            if(this.storeSelectList.some(item=>item.id==val)){
              storeIds.push( val) 
            ...
            this.ruleForm = res;
            this.ruleForm.storeId=storeIds
            this.getAccInfo( this.ruleForm.uid)
            this.loading = false
          }).catch(res=>{
            this.loading = false
        ...
        editForm:Debounce(function(name){
          this.$refs[name].validate((valid) => {
            if (valid) {
              ...
              storeStaffUpdateApi({id:this.ruleForm.id},this.ruleForm).then(async () => {
