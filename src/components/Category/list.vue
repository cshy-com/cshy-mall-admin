        ...
        <el-card class="box-card">
              ...
              <el-form inline size="small">
                ...
                <el-form-item label="名称：">
                  <el-input
                    ...
                    class="selWidth"
                    size="small"
                    clearable @keyup.enter.native="handlerGetList" 
                  >
                    <el-button
                      slot="append"
                      icon="el-icon-search"
          ...
          >
            ...
            <u-table-column
              ...
              label="名称"
              :width="300"
              style="display: flex" class-name="tab-title"
            >
              <template slot-scope="scope">
                {{ scope.row.name }} | {{ scope.row.id }}
              </template>
            ...
            <template v-if="!selectModel">
              <u-table-column prop="type" label="类型" :width="200">
                <template slot-scope="scope">
                  <span>{{
                    scope.row.type | filterCategroyType | filterEmpty
                  }}</span>
                </template>
              </u-table-column>
              <u-table-column prop="listPic" label="分类图标" :width="200">
                <template slot-scope="scope">
                  <div class="listPic" v-if="biztype.value === 5">
                    <i
              ...
              <u-table-column label="Url" min-width="250" v-if="biztype.value === 5" key="2">
                <template slot-scope="scope">
                  <span>{{ scope.row.url }}</span>
                </template>
              </u-table-column>
              <u-table-column prop="sort" label="排序" :width="200">
              </u-table-column>

              <u-table-column label="状态" min-width="150">
                <template
                  slot-scope="scope"
              ...
              <u-table-column label="操作" min-width="200"  >
                <template slot-scope="scope">
      ...
      this.biztype.value !== 3
        ? categoryApi
            ...
            .then((data) => {
              ...
              this.$refs.plTreeTable.reloadData(this.treeList);
              this.loading = false;
            })
            .catch(() => {
              this.loading = false;
